...
export function renderInputDictionary(): string {
  ...
  w('they drive the candidate-additions list below. Fields already in the')
  w('published worker-portal contract are marked ✦.')
  w()
  ...
  for (const group of data.groups) {
    ...
    for (const e of group.entries) {
      ...
      if (e.inPublishedContract) meta.push('✦ in published contract')
      w(meta.join(' · '))
      w()
      if (e.definition) {
