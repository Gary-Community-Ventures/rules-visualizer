...
export function renderInputDictionary(): string {
  ...
  w('# Input dictionary — v2 field-grouping proposal')
  w()
  ...
  w('> gets a "not a recognised field" warning. For the fields the API actually')
  w('> accepts — names, types, enum values — use the')
  w('> **[engine-input catalog](https://gary-community-ventures.github.io/rules-visualizer/engine-inputs.html)**')
  w('> (machine-readable: `engine-inputs.json`). This document remains useful for')
  w('> its rule-authored *definitions*, policy citations, and the field-grouping/')
  w('> consolidation proposals under discussion. Note also that the live v2')
  w('> responses call the still-needed list `missingInputs`; `missingInformation`')
  w('> below is the v1 name.')
  w()
  ...
  w('the regulations — so the *definitions* cannot drift from what the rules')
