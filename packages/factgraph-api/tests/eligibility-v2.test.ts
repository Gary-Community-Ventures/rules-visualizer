...
test('per-member missingInputs — each member gets only their own unresolved fields', async () => {
  // head has dateOfBirth; spouse does not. The attribution should surface
  ...
  // head provided dateOfBirth → should not be in their missing list
  assert.ok(
    !headFields.includes('dateOfBirth'),
    `head should not need dateOfBirth; got: ${headFields.join(', ')}`
  )
  // spouse did not → should appear in their missing list
  assert.ok(
