...
function systemPrompt(ctx: ChatContext): string {
  return [
    ...
    `You can use tools to look up nodes, search, and explore dependencies. Start with the direct answer to the question. Keep answers to 2 sentences or fewer. Reference specific node names so the user can click them. Don't wrap node names in backticks or code formatting — just write them as plain text.`,
