...
- **`jvm`**: The JVM Scala build can't run anything with `^` in the XML. Two layers of problem:
  1. `FactDictionary.fromXml` validates paths at freeze-time → throws _"cannot find fact at path '^'"_. The JVM harness (`jvm/Harness.scala:rewriteCaretPaths`) preprocesses these into absolute paths to clear this layer.
